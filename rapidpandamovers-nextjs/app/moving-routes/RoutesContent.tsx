...
export default function RoutesContent({ currentPage, fromLocation }: RoutesContentProps) {
  ...
  const getPageUrl = (page: number) => {
    ...
    let base: string;
    if (fromLocation) {
      base = page === 1 ? `/moving-routes/${fromLocation}` : `/moving-routes/${fromLocation}/page/${page}`;
    } else {
      base = page === 1 ? '/moving-routes' : `/moving-routes/page/${page}`;
    }
            ...
            {totalPages > 1 && (
              <>
                ...
                <p className="text-center text-gray-500 mt-4">
                  Page {currentPage} of {totalPages} ({allFilteredRoutes.length} routes)
                </p>
              </>
            )}
          </div>
        ...
        <div className="pt-20">
          <div className="container mx-auto text-center">
            <p className="text-xl text-gray-600 mb-4">
              No routes found for &ldquo;{titleCase(fromLocation)}&rdquo;
            </p>
            <Link
              href="/moving-routes"
