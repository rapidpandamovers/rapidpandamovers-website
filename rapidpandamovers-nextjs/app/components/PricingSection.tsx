            ...
            <div key={index} className={`bg-white border rounded-lg p-6 text-center hover:shadow-lg transition-shadow relative ${category.popular ? 'border-orange-500 shadow-lg' : 'border-gray-200'}`}>
              {category.popular && (
                <div className="absolute -top-3 left-1/2 transform -translate-x-1/2">
                  <span className="bg-orange-700 text-white px-3 py-1 rounded-full text-sm font-medium">{ui.pricing.mostPopular}</span>
                </div>
              )}
              ...
              {category.cost && (
                <div className="mb-3">
                  <p className="text-2xl font-bold text-orange-700">${category.cost.toLocaleString(undefined, { maximumFractionDigits: 2 })}</p>
              ...
              <ul className="space-y-2 mb-6">
                {category.features.map((feature, i) => (
                  <li key={i} className="text-sm text-gray-600 flex items-center justify-center">
                    <CheckCircle className="w-4 h-4 text-orange-700 mr-2" />
                    {feature}
                  </li>
                ))}
              </ul>
              <Link
                ...
                className={`block w-full py-3 px-4 rounded-lg font-semibold transition-colors text-center ${category.popular ? 'bg-orange-700 hover:bg-orange-800 text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-800'}`}
              >
